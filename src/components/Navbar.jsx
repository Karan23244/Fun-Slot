...
const Navbar = ({ onStartNow }) => {
  ...
  return (
    <nav
      ...
      {/* 🔥 Top Header */}
      <div className="w-full text-gray-300 text-[10px] sm:text-xs md:text-sm py-2 sm:py-3 px-4 sm:px-6 lg:px-12 flex flex-wrap items-center justify-between gap-2 border-b border-purple-900/30">
        {/* Left: 18+ Badge */}
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full border-2 border-red-500 text-red-500 font-bold text-xs sm:text-sm">
            18+
          </div>
          <span className="text-sm sm:text-lg md:text-xl text-gray-300 font-medium">
            Adults Only
          </span>
        </div>
          ...
          <div className="hidden md:flex items-center gap-6">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="text-gray-400 hover:text-white text-sm font-medium transition hover:text-purple-300">
                {link.label}
              </a>
            ))}
          </div>
        ...
        <div className="px-4 py-4 space-y-2 border-t border-purple-900/30">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="block py-2 px-3 text-gray-300 hover:text-white rounded-lg hover:bg-purple-900/20"
              onClick={() => setMobileOpen(false)}>
              {link.label}
            </a>
          ))}
